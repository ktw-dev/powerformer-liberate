...
| `cpu`                                      | `self`, `ct`                                                                                               | Shortcut for moving a `data_struct` to the CPU.                                                                                         | `data_struct`를 CPU로 이동하는 단축 메서드. |
| `cuda`                                     | `self`, `ct`                                                                                               | Shortcut for moving a `data_struct` to the GPU(s).                                                                                        | `data_struct`를 GPU로 이동하는 단축 메서드. |
...
| `save`                                     | `self`, `text`, `filename=None`                                                                          | Saves a `data_struct` object to a file using pickle. Moves to CPU if on GPU.                                                                 | `data_struct` 객체를 pickle을 사용하여 파일로 저장한다. GPU에 있는 경우 CPU로 이동한다. |
| `load`                                     | `self`, `filename`, `move_to_gpu=True`                                                                     | Loads a `data_struct` object from a pickled file. Optionally moves to GPU.                                                                 | pickle된 파일에서 `data_struct` 객체를 로드한다. 선택적으로 GPU로 이동한다. |
